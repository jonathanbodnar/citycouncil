-- Update payout triggers to only create payouts when video is DELIVERED
-- Payout is created when video_url is set, not just when status is 'completed'

-- 1. Drop existing trigger
DROP TRIGGER IF EXISTS trigger_create_payout_on_completion ON public.orders;

-- 2. Update the function to trigger on video delivery
...
COMMENT ON FUNCTION create_payout_on_order_completion() IS 
'Creates payout record when video is delivered (video_url is set) for completed orders. 
Only triggers once per order. Updates talent earnings and weekly batches.';
