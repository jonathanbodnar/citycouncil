-- Super simple: Create demo order using an EXISTING user as the customer
...
BEGIN
  ...
  -- Use ANY existing user as the demo customer (just grab the first one that's not the talent)
  SELECT id, full_name, email INTO existing_user_record
  FROM users
  ...
  demo_user_id := existing_user_record.id;
  RAISE NOTICE '✅ Using existing user as demo customer: % (% - ID: %)', 
    existing_user_record.full_name, existing_user_record.email, demo_user_id;
  ...
  RAISE NOTICE '📧 Customer: % (%)', existing_user_record.full_name, existing_user_record.email;
