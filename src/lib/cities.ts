...
export const NORTH_TEXAS_CITIES = [
  {
    slug: 'frisco',
    name: 'Frisco',
    state: 'TX',
...
export const ZIP_CODE_TO_CITY_SLUGS: Map<string, string[]> = new Map();

NORTH_TEXAS_CITIES.forEach(city => {
  city.zipCodes.forEach(zip => {
    const existing = ZIP_CODE_TO_CITY_SLUGS.get(zip) || [];
    existing.push(city.slug);
    ZIP_CODE_TO_CITY_SLUGS.set(zip, existing);
  });
});

// Get all unique zip codes
export const ALL_ZIP_CODES = Array.from(ZIP_CODE_TO_CITY_SLUGS.keys()).sort();

// Find city slugs by zip code
export function getCitySlugsByZipCode(zipCode: string): string[] {
  return ZIP_CODE_TO_CITY_SLUGS.get(zipCode) || [];
}

// Check if zip code is in our coverage area
export function isZipCodeCovered(zipCode: string): boolean {
  return ZIP_CODE_TO_CITY_SLUGS.has(zipCode);
}

// Get all city slugs for nearby search (North Texas area)
export function getNearbyCitySlugs(zipCode: string): string[] {
  if (isZipCodeCovered(zipCode)) {
    return getCitySlugsByZipCode(zipCode);
  }
  
  // Check if it's in the general North Texas area (75xxx or 76xxx)
  if (zipCode.startsWith('75') || zipCode.startsWith('76')) {
    return NORTH_TEXAS_CITIES.map(c => c.slug);
  }
