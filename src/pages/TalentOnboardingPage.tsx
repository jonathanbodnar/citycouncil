...
const TalentOnboardingPage: React.FC = () => {
  ...
  const handleOtpSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Link talent profile to user AND create user record in one RPC call
      // This uses SECURITY DEFINER to bypass RLS
      console.log('🔗 Linking talent profile to user (with user creation):', {
        talentId: talentId,
        userId: user.id,
        token: token,
        email: normalizedEmail
      ...
      const { data: linkResult, error: linkError } = await supabase.rpc('link_talent_profile_to_user', {
        ...
        p_avatar_url: onboardingData?.talent?.temp_avatar_url || null
      });
            ...
            try {
              ...
              if (watermarkError) {
                console.error('❌ Watermark error details:', {
                  ...
                  context: watermarkError.context,
                  videoUrl: uploadResult.videoUrl
                });
                toast.dismiss('watermark');
                toast.success('Video uploaded! (Watermark will be added later)');
                ...
                console.error('⚠️ No watermarked URL returned:', watermarkData);
                toast.dismiss('watermark');
                toast.success('Video uploaded! (Watermark will be added later)');
              ...
              console.error('❌ Watermark exception:', {
                error: watermarkErr,
                message: watermarkErr.message,
                stack: watermarkErr.stack,
                videoUrl: uploadResult.videoUrl
              });
              toast.dismiss('watermark');
              toast.success('Video uploaded! (Watermark will be added later)');
