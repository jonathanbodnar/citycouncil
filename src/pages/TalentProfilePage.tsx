        ...
        <div className="md:flex rounded-3xl overflow-hidden relative h-full">
          ...
          <div className="md:w-1/3 relative z-10 md:self-stretch">
            <div className="aspect-square md:aspect-auto md:h-full bg-gray-100 relative group">
              {playingPromoVideo && talent.promo_video_url ? (
                ...
                <>
                  ...
                  {talent.promo_video_url && (
                    ...
                    >
                      <div className="bg-black/20 backdrop-blur-[2px] p-2 rounded-full border border-white/20 hover:bg-black/30 hover:scale-105 transition-all duration-300">
                        <PlayIcon className="h-5 w-5 text-white/50" />
                      </div>
                    </button>
                  )}
                </>
              ...
              {/* Mobile Social Links - Bottom right of image */}
              <div className="absolute bottom-3 right-3 md:hidden flex items-center gap-1 bg-black/40 backdrop-blur-sm rounded-full px-2 py-1">
                {(() => {
                  const twitterHandle = 
                  ...
                  const rumbleType = (talent as any).rumble_type || 'c';
                  const youtubeHandle = 
                  ...
                  return (
                    <>
                      {twitterHandle && (
                        <a href={`https://x.com/${twitterHandle}`} target="_blank" rel="noopener noreferrer" className="p-1 text-white/60 hover:text-white/90">
                          <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
                        </a>
                      )}
                      {instagramHandle && (
                        <a href={`https://instagram.com/${instagramHandle}`} target="_blank" rel="noopener noreferrer" className="p-1 text-white/60 hover:text-white/90">
                      ...
                      {facebookHandle && (
                        <a href={`https://facebook.com/${facebookHandle}`} target="_blank" rel="noopener noreferrer" className="p-1 text-white/60 hover:text-white/90">
                          <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                        </a>
                      )}
                      {tiktokHandle && (
                        <a href={`https://tiktok.com/@${tiktokHandle}`} target="_blank" rel="noopener noreferrer" className="p-1 text-white/60 hover:text-white/90">
                      ...
                      {rumbleHandle && (
                        <a href={`https://rumble.com/${rumbleType}/${rumbleHandle.replace('@', '')}`} target="_blank" rel="noopener noreferrer" className="p-1 text-white/60 hover:text-white/90">
                          <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24"><path d="M14.4528 13.5458c0.8064 -0.6542 0.9297 -1.8381 0.2756 -2.6445a1.8802 1.8802 0 0 0 -0.2756 -0.2756 21.2127 21.2127 0 0 0 -4.3121 -2.776c-1.066 -0.51 -2.256 0.2 -2.4261 1.414a23.5226 23.5226 0 0 0 -0.14 5.5021c0.116 1.23 1.292 1.964 2.372 1.492a19.6285 19.6285 0 0 0 4.5062 -2.704v-0.008zm6.9322 -5.4002c2.0335 2.228 2.0396 5.637 0.014 7.8723A26.1487 26.1487 0 0 1 8.2946 23.846c-2.6848 0.6713 -5.4168 -0.914 -6.1662 -3.5781 -1.524 -5.2002 -1.3 -11.0803 0.17 -16.3045 0.772 -2.744 3.3521 -4.4661 6.0102 -3.832 4.9242 1.174 9.5443 4.196 13.0764 8.0121v0.002z"/></svg>
                        </a>
                      )}
                      {youtubeHandle && (
                        <a href={`https://youtube.com/@${youtubeHandle.replace('@', '')}`} target="_blank" rel="noopener noreferrer" className="p-1 text-white/60 hover:text-white/90">
                          <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24"><path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg>
                        </a>
                      )}
                      <button onClick={() => setShareModalOpen(true)} className="p-1 text-white/60 hover:text-white/90">
          ...
          <div className="md:w-2/3 p-4 md:p-5 relative z-10 flex flex-col">
            ...
            {/* Social Links - Desktop only (mobile shows on image) */}
